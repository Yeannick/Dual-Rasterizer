...
//----------------------------------------------------
float3 SampleTexture(SamplerState mySampler, float2 UV)
{
...
{
	float3 Color = Shading(input,samPoint);
...
float4 PSLinear(VS_OUTPUT input) : SV_TARGET
{
	...
	float3 Color = Shading(input,samLinear);
...
float4 PSAnisotropic(VS_OUTPUT input) : SV_TARGET
{
	...
	float3 Color = Shading(input,samAnisotropic);
